...
export const PokeDex = () => {
            ...
            const detailedPokemon = await Promise.all(
                data.results.map(async (index) => {
                    ...
                    return {
                        ...
                        Image: sprites.front_default, Image2: sprites.other['official-artwork'].front_default,
    ...
    const canGoNextSearch = searchResult && (currentPageSearch * itemsPerPage) < searchResult.length;
    const totalPagesSearch = Math.ceil((searchResult || []).length / itemsPerPage);
    ...
    const handlePageChangeSearch = (page) => {
        if (page >= 1 && page <= totalPagesSearch) {
            setCurrentPageSearch(page);
        }
    };
    
    const getPageRangeSearch = () => {
        const startPageSearch = Math.max(1, currentPageSearch - 4);
        const endPageSearch = Math.min(totalPagesSearch, currentPageSearch + 4);
        ...
        <div>
            <nav className="fixed left-0 w-full bg-gradient-to-r from-gray-950 to-red-900 p-3 z-10 top-0">
                    <div className="flex justify-between">
                        ...
                        <div className="relative text-white font-extrabold text-4xl">
                            ...
                            <span className="relative">PokeDex Alpha</span>
                            </div>
                        </div>
                        ...
                        transition-colors duration-500 hover:bg-gradient-to-r hover:from-orange-400 hover:to-orange-500 hover:text-white">Buscar</button>
                        </div>
                    </div>
                </nav>
            <BackGroundeffect1/>
            ...
            <Loader />
                ...
                scrollbarWidth: "none",
                 ...
                 <section   className={`flex flex-wrap justify-start aligncontent-start gap-6 pl-7 pt-16 mt-4 overflow-y-auto`}     style={{
                            height: "calc(100vh - 5rem)", // Altura dinámica del contenedor
                            alignContent: "start", // Alinea filas en la parte superior
                            justifyContent: "start" // Alinea elementos al inicio de cada fila
                        }}>
                ...
                <div
                    ...
                    className={`bg-gradient-to-bl from-red-200 to-red-400 bg-fixed bg-cover rounded-lg shadow-lg p-1
                        ...
                        animate__animated animate__zoomIn cursor-pointer flex flex-col items-center max-w-[330px] min-w-[300px] w-[310px] h-[160px]`}
                >
            ...
            <footer className="flex justify-center fixed bottom-2 left-1/2 transform -translate-x-1/2 z-10 ">
                <button
                    ...
                    className={`bg-gradient-to-r from-red-500 to-red-600 text-white font-bold py-2 px-4 rounded-full shadow-lg focus:outline-none m-1
                        ${currentPageSearch > 1 ? 'hover:ring-2 hover:ring-orange-200 transition-colors duration-500 hover:bg-gradient-to-r hover:from-orange-400 hover:to-orange-500 hover:text-white' : ''} `}
                >
                    Anterior
                </button>
                {getPageRangeSearch().map(page => (
                    <button
                        ...
                        className={`${
                            page === currentPageSearch ? 'bg-red-600 text-white' : 'bg-gray-200 text-black hover:bg-orange-600 hover:text-white'
                ...
                <button
                    ...
                    className="bg-gradient-to-l from-red-500 to-red-600 text-white font-bold py-2 px-4 rounded-full shadow-lg focus:outline-none hover:ring-2 hover:ring-orange-200 m-1
                    transition-colors duration-500 hover:bg-gradient-to-r hover:from-orange-400 hover:to-orange-500 hover:text-white"
                >
                    Siguiente
                </button>
            </footer>
            ...
            <>
            <section className={`grid ${(itemsPerPage <= 9 ? "grid-cols-3 grid-rows-3" : "grid-cols-3")} h-[calc(100vh-5rem)] pt-16 overflow-y-auto`} style={{
                scrollbarWidth: "none",
            }}>
                     {pokemon.map(index => {
                ...
                <button
                    ...
                    className={`bg-gradient-to-r from-red-500 to-red-600 text-white font-bold py-2 px-4 rounded-full shadow-lg focus:outline-none m-1
                    ${currentPage > 1 ? 'hover:ring-2 hover:ring-orange-200 transition-colors duration-500 hover:bg-gradient-to-r hover:from-orange-400 hover:to-orange-500 hover:text-white' : ''} `}
                >
                    Anterior
                </button>
                {getPageRange().map(page => (
                    <button
                        ...
                        className={`${
                            page === currentPage ? 'bg-red-600 text-white' : 'bg-gray-200 text-black hover:bg-orange-600 hover:text-white'
                        } h-10 w-10 text-center font-bold rounded-full shadow-lg m-1`}
                    >
                        {page}
                    </button>
